...
def process_cfg_for_version_compat(filebeat_cfg):

    major, minor = get_filebeat_versions()

    # Versions less than 1.6 do not support external health checks
    # Removed those configuration times
    if major < 2 and minor < 6:
        LOG.warning("Found %(major)s.%(minor)s version of filebeat. "
                    "Disabling external checks. Health monitor of type "
                    "PING will revert to TCP.",
                    {'major': major, 'minor': minor})
        filebeat_cfg = re.sub(r" * ?.*external-check ?.*\s", "", filebeat_cfg)
